		...
		public function enqueue_assets() {
			...
			if ( $footer_content_block !== '' ) {
				...
				if ( function_exists( 'icl_object_id' ) ) {
					$icl_object_id = icl_object_id( $footer_content_block, G5CORE()->cpt()->get_content_block_post_type(), false );
					if ( $icl_object_id !== false ) {
						$footer_content_block = $icl_object_id;
					}
				}
			...
			if ( $page_title_content_block !== '' ) {
				if ( function_exists( 'icl_object_id' ) ) {
					$icl_object_id = icl_object_id( $page_title_content_block, G5CORE()->cpt()->get_content_block_post_type(), false );
					if ( $icl_object_id !== false ) {
						$page_title_content_block = $icl_object_id;
					}
				}
			...
			if ( is_404() ) {
				...
				if ( $content_404_block !== '' ) {
					if ( function_exists( 'icl_object_id' ) ) {
						$icl_object_id = icl_object_id( $content_404_block, G5CORE()->cpt()->get_content_block_post_type(), false );
						if ( $icl_object_id !== false ) {
							$content_404_block = $icl_object_id;
						}
					}
				...
				if ( is_object( $menu ) ) {
					...
					foreach ( (array) $nav_items as $nav_item ) {
						if ( G5CORE()->cpt()->get_xmenu_mega_post_type() == $nav_item->object ) {
							...
							if ( function_exists( 'icl_object_id' ) ) {
								$icl_object_id = icl_object_id( $object_id, G5CORE()->cpt()->get_xmenu_mega_post_type(), false );
								if ( $icl_object_id !== false ) {
