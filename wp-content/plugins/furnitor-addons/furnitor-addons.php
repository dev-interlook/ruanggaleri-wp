<?php
/**
 *    Plugin Name: Furnitor Addons
 *    Plugin URI: http://g5plus.net
 *    Description: Addons For Furnitor Theme
 *    Version: 1.0.0
 *    Author: G5
 *    Author URI: http://g5plus.net
 *
 *    Text Domain: furnitor-addons
...
if (!class_exists('G5ThemeAddons')) {
	class G5ThemeAddons {
		...
		public function admin_notices() {
			?>
			<div class="error">
				<p><?php esc_html_e('Furnitor Addons is enabled but not effective. It requires G5 Core in order to work.','furnitor-addons'); ?></p>
			</div>
			<?php
		}
		...
		public function load_text_domain() {
			load_plugin_textdomain( 'furnitor-addons', false, $this->plugin_dir('languages'));
		}
		...
		public function get_plugin_template($slug, $args = array()) {
			...
			if (!file_exists($located)) {
				_doing_it_wrong(__FUNCTION__, sprintf('<code>%s</code> does not exist.', $slug), '1.0.0');
