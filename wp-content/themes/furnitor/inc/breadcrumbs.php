...
function furnitor_the_breadcrumbs() {
	...
	if (is_singular()) {
		...
		if (isset($post->post_type) && get_post_type_archive_link($post->post_type)) {
			$html_markup .= furnitor_get_post_type_archive();
		}

		$html_markup .= furnitor_get_breadcrumb_leaf_markup();
	} else {
		// Blog page is a dedicated page.
		if ( is_home() && ! is_front_page() ) {
			$posts_page         = get_option( 'page_for_posts' );
			$posts_page_title   = get_the_title( $posts_page );
			$html_markup .= furnitor_get_single_breadcrumb_markup( $posts_page_title );
		}

		// Custom post types archives.
		if ( is_post_type_archive() ) {
			$html_markup .= furnitor_get_post_type_archive( false );

			// Search on custom post type (e.g. Woocommerce).
			if ( is_search() ) {
				$html_markup .= furnitor_get_breadcrumb_leaf_markup( 'search' );
			}
		} // Taxonomy Archives.
		elseif ( is_tax() || is_tag() || is_category() ) {
			if ( is_tag() ) { // If we have a tag archive, add the tag prefix.
				$html_markup .= esc_html__('Tag:','furnitor');
			}
			$html_markup .= furnitor_get_taxonomies();
			$html_markup .= furnitor_get_breadcrumb_leaf_markup( 'term' );
		} // Date Archives.
		elseif ( is_date() ) {
			global $wp_locale;
			// Set variables.
			...
			if ( is_year() ) {
				$html_markup .= furnitor_get_breadcrumb_leaf_markup( 'year' );
			} // Month Archive, needs year link and month leaf.
			elseif ( is_month() ) {
				$html_markup .= furnitor_get_single_breadcrumb_markup( $year, get_year_link( $year ) );
				$html_markup .= furnitor_get_breadcrumb_leaf_markup( 'month' );
			} // Day Archive, needs year and month link and day leaf.
			elseif ( is_day() ) {
				$month      = get_the_date('m');
				$month_name = $wp_locale->get_month( $month );
				$html_markup .= furnitor_get_single_breadcrumb_markup( $year, get_year_link( $year ) );
				$html_markup .= furnitor_get_single_breadcrumb_markup( $month_name, get_month_link( $year, $month ) );
				$html_markup .= furnitor_get_single_breadcrumb_markup( 'day' );
			}
		} // Author Archives.
		elseif ( is_author() ) {
			$html_markup .= furnitor_get_breadcrumb_leaf_markup( 'author' );
		} // Search Page.
		elseif ( is_search() ) {
			$html_markup .= furnitor_get_breadcrumb_leaf_markup( 'search' );
		} // 404 Page.
		elseif ( is_404() ) {
			$html_markup .= furnitor_get_breadcrumb_leaf_markup( '404' );
		}
	}
	if ($html_markup) {
		$html_markup = '<ul class="breadcrumbs d-flex flex-wrap">' . $html_markup . '</ul>';
	}
	echo wp_kses($html_markup,wp_kses_allowed_html('breadcrumbs'));
}

function furnitor_get_breadcrumb_home() {
	if (!is_front_page()) {
		$output = furnitor_get_single_breadcrumb_markup(esc_html__('Home','furnitor'), get_home_url('/'));;
	} else {
		$output = furnitor_get_single_breadcrumb_markup(esc_html__('Blog','furnitor'));
	}
	return $output;
}

function furnitor_get_single_breadcrumb_markup($title, $link = '', $separator = true, $micro_data = true)
{
...
function furnitor_get_woocommerce_shop_page($linked = true) {
	$post_type = 'product';
	...
	if (isset($post_type_object) && class_exists('WooCommerce') && (is_woocommerce() || is_cart() || is_checkout() || is_account_page())) {
		...
		$shop_page_markup = furnitor_get_single_breadcrumb_markup($shop_page_name, $link);
	}

	return $shop_page_markup;
}

function furnitor_get_post_type_archive($linked = true) {
	global $wp_query;
	...
	if (($post_type === 'product') && class_exists('WooCommerce')) {
		$woocommerce_shop_page = furnitor_get_woocommerce_shop_page($linked);
	...
	if ($post_type === 'post') {
		$archive_title = esc_html__( 'Blog', 'furnitor' );
	}

	return furnitor_get_single_breadcrumb_markup($archive_title, $link);
}

function furnitor_get_breadcrumb_leaf_markup($object_type = '') {
	...
	switch ( $object_type ) {
		...
		case 'author':
			...
			break;
		case 'search':
			$title = esc_html__('Search:','furnitor') . ' ' . esc_html( get_search_query() );
			break;
		case '404':
			$title = esc_html__('Page Not Found','furnitor');
			break;
		default:
...
function furnitor_get_taxonomies() {
	global $wp_query;
	$term = $wp_query->get_queried_object();
	$terms_markup = '';
	...
	if ( 0 != $term->parent && is_taxonomy_hierarchical( $term->taxonomy ) ) {
		...
		foreach ( $term_ancestors as $term_ancestor ) {
			...
			$terms_markup .= furnitor_get_single_breadcrumb_markup( $term_object->name, get_term_link( $term_object->term_id, $term->taxonomy ) );
		}
	}
